...
contract SemaphoreAirdropTest is DSTest {
    ...
    function setUp() public {
        ...
        airdrop = new SemaphoreAirdrop(semaphore, groupId, nft, address(user), 1 ether);

        hevm.label(address(this), 'Sender');
        hevm.label(address(user), 'Holder');
        // hevm.label(address(token), 'Token');
        hevm.label(address(nft), 'Nft');
        ...
        tokenId = nft.mintTo{value: 10 ether} (address(user));

        // Approve spending from the airdrop contract
        hevm.prank(address(user));
        nft.approve(address(airdrop), tokenId);
    }
    ...
    function testCanClaim() public {
        ...
        airdrop.claim(address(this), tokenId, semaphore.getRoot(groupId), nullifierHash, proof);
